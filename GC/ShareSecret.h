...
#include "Protocols/MultiReplicated.h"
#include "Protocols/ReplicatedMC.h"
#include "Protocols/MultiReplicatedMC.h"
...
public:
    ...
    BitVec local_mul_0(const ReplicatedSecret& other) const;
    BitVec local_mul_1(const ReplicatedSecret& other) const;
    BitVec local_mul_2(const ReplicatedSecret& other) const;
...
class SemiHonestMultiRepPrep;

class SemiHonestMultiRepSecret : public ReplicatedSecret<SemiHonestMultiRepSecret>
{
    typedef ReplicatedSecret<SemiHonestMultiRepSecret> super;

public:
    typedef Memory<SemiHonestMultiRepSecret> DynamicMemory;

    typedef MultiReplicatedMC<SemiHonestMultiRepSecret> MC;
    typedef MultiReplicated<SemiHonestMultiRepSecret> Protocol;
    typedef MC MAC_Check;
    typedef SemiHonestMultiRepPrep LivePrep;
    typedef MultiReplicatedInput<SemiHonestMultiRepSecret> Input;

    typedef SemiHonestMultiRepSecret part_type;
    typedef SemiHonestMultiRepSecret small_type;
    typedef SemiHonestMultiRepSecret whole_type;

    static MC* new_mc(mac_key_type) { return new MC; }

    SemiHonestMultiRepSecret() {}
    template<class T>
    SemiHonestMultiRepSecret(const T& other) : super(other) {}
};
